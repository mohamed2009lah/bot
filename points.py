...
class PointsSystem:
    def __init__(self):
        ...
        self.daily_bonus = 2
        self.stars_ratio = 10  # 1 نجمة = 10 نقاط
    ...
    def init_tables(self):
        ...
        if c.fetchone()[0] == 0:
            prices = [
                (10, 0.50, 5),   # 10 نقاط = 0.50$ أو 5 نجوم
                (25, 1.00, 10),
                (50, 1.80, 18),
                (100, 3.00, 30),
                (250, 6.50, 65),
    ...
    def buy_points_with_stars(self, user_id, package_amount):
        """شراء النقاط بالنجوم، ترجع عدد النجوم المطلوب"""
        c = get_conn().cursor()
        ...
        stars = row[0]
        c.connection.close()
        self.add_points(user_id, package_amount, 'buy_stars', f'شراء {package_amount} نقطة بالنجوم')
        return stars, None
    ...
    def get_user_stats(self, user_id):
        ...
        row = c.fetchone()
        c.connection.close()
        return {'balance': row[0], 'total_earned': row[1], 'total_spent': row[2]} if row else {'balance': 0, 'total_earned': 0, 'total_spent': 0}
